...
from data_manager import get_input_dir, normalize_date_str
...
def process_to_df(json_data):
    ...
    try:
        ...
        filtered = json_data.get("filtered", {})
        expiry_dates = records.get("expiryDates", [])
        
        # Determine Ground-Truth Spot Price (Multi-Gate Fallback)
        # 1. First check top-level records, then filtered, then global json_data, then first data item
        spot_price = json_data.get("underlyingValue") or \
                     records.get("underlyingValue") or \
                     filtered.get("underlyingValue") or \
                     (records.get("data", [{}])[0].get("underlyingValue") if records.get("data") else 0) or 0
...
def fetch_and_save(symbol: str = "NIFTY", manual_cookie: Optional[str] = None, curl_command: Optional[str] = None) -> Tuple[bool, str, Optional[str]]:
    ...
    if curl_command:
        success, msg, target = _execute_mirror_sync(symbol, curl_command)
        if success:
            return success, msg, target
        logger.warning(f"Mirror Mode Silent/Failed: {msg}. Falling back to Automated Layers...")
    ...
    try:
        ...
        if not expiry_dates:
            ...
            if filtered:
                 ...
                 # Check if we are in Pre-Open window (8:00 - 9:15 AM IST)
                 now = datetime.now().time()
                 if now >= dtime(8, 0) and now < dtime(9, 15):
                      return True, "⌛ NSE Pre-Open: Awaiting Live Market Data Generation", "Pre-Open"
                 
                 return False, f"JSON Error: 'records.expiryDates' is empty. Keys: {list(raw_data.keys())}", None
        
        target_expiry = normalize_date_str(expiry_dates[0]) if expiry_dates else "Latest"
        df = process_to_df(raw_data)
        if df is not None:
            input_dir = get_input_dir(expiry_date=target_expiry)
            ...
            try:
                ...
                # 1. Save Raw JSON to MongoDB
                # Use ISO format for MongoDB timestamp
                iso_ts = datetime.now().isoformat()
                db.save_raw_snapshot(raw_data, target_expiry, iso_ts)
                
                # 2. Extract Metrics for MySQL
                # Replicate the logic from db_sync.py / app.py
                spot = df["Spot Price"].iloc[0] if "Spot Price" in df.columns else 0
                if spot > 0:
                    # PCR Bands
                    pcr_bands = {}
                    ...
                    pcr_overall = compute_pcr(df, "Strike Price", "CE OI", "PE OI")["overall_pcr"]
                    sr_res = compute_support_resistance(df, "Strike Price", "CE OI", "PE OI", top_n=5, smooth_window=3)
                ...
                db.close()
                logger.info(f"Successfully persisted snapshot and analysis for {target_expiry} to Databases.")
